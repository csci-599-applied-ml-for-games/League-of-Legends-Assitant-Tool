...
from conf.ProfileModelLabel import LABEL_NAME, BANNED_LABEL_NAME, MINI_MAP_CHAMP_LIST
from conf.Settings import IN_GAME_PROFILE_RECOGNITION_CHECK_POINT_PATH, \
    BANNED_CHAMP_RECOGNITION_CHECK_POINT_PATH, \
    MINI_MAP_CHAMP_RECOGNITION_CHECK_POINT_PATH
...
class MiniMapChampionModel(object):
    ...
    def __init__(self):
        self._reload_model_schema()
        self.model.load_weights(MINI_MAP_CHAMP_RECOGNITION_CHECK_POINT_PATH)
    ...
    def predictSingleImg(self, image):
        ...
        image = np.expand_dims(image, axis=0)
        predict_result = self.model.predict(image)
        return MINI_MAP_CHAMP_LIST[np.argmax(predict_result, axis=1)[0]]
    ...
    def predictImgs(self, images):
        ...
        for img in images:
            ...
            image = np.expand_dims(image, axis=0)
            predict_result = self.model.predict(image)
            results.append(MINI_MAP_CHAMP_LIST[np.argmax(predict_result, axis=1)[0]])

        return results

    @classmethod
    def getInstance(cls, *args, **kwargs):
        if not hasattr(MiniMapChampionModel, "_instance"):
            with MiniMapChampionModel._instance_lock:
                if not hasattr(MiniMapChampionModel, "_instance"):
                    MiniMapChampionModel._instance = MiniMapChampionModel(*args, **kwargs)
        return MiniMapChampionModel._instance
